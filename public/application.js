...
var authApi = __webpack_require__(3);

var playerTurn = 0; // to figure out who starts
var currentPlayer = 'x'; // assign a string to the player
...
var afterWin = function afterWin(event) {
...
var didAnyoneWin = function didAnyoneWin(event) {
...
var authEvents = __webpack_require__(7); // your JS code goes here
var authLogic1 = __webpack_require__(2);
...
var onCreateGame = function onCreateGame(event) {
  event.preventDefault();
  authApi.createGame(event).then(authUi.createGameSuccess).catch(authUi.createGameFail);
  authLogic1.resetGame();
};

var onUpdateGame = function onUpdateGame(event) {
  event.preventDefault();
  authApi.updateGame(event).then(authUi.updateGameSuccess).catch(authUi.updateGameFail);
};

var onGetGames = function onGetGames(event) {
  event.preventDefault();
  authApi.getGames(event).then(authUi.getGamesSuccess).catch(authUi.getGamesFail);
};
module.exports = {
  onSignUp: onSignUp,
  onSignIn: onSignIn,
...
var changePasswordFail = function changePasswordFail(response) {
  $('#content').html('Sorry, you were not able to change your password,', 'Please try again !');
  $('#change-password-form').find('input').val('');
  $('#sign-up-form').find('input').val('');
  $('#sign-in-form').find('input').val('');
};
...
// imports
exports.push([module.i, "@import url(http://fonts.googleapis.com/css?family=Roboto+Slab|Open+Sans:400italic,700italic,400,700);", ""]);
